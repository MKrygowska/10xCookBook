...
async function review(diff: string): Promise<Review> {
  ...
  if (!apiKey) {
    throw new Error("Brak klucza API. Ustaw zmienną środowiskową GEMINI_API_KEY.");
  }
...
\`\`\``;
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3.1-flash-lite", // Używamy modelu gemini-3.1-flash-lite, który działa na Twoim kluczu API bez limitów i przeciążeń
